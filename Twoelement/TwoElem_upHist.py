...
WWW = FunctionSpace(mesh, 'DG', 0)     
HistOld, HistUpdate = Function(WWW), Function(WWW) 
#--------------------------------------------------------
#    Boundary Conditions
#-------------------------------------------------------
...
phi_old,phi_conv, phi_new = Function(V),Function(V), Function(V)
...
E_du = ((1 - phi_new)**2 + 1e-6)*inner(grad(v),sigma(u))*dx

E_phi = ( Gc*l_o*inner(grad(p),grad(q))+\
            ((Gc/l_o) + 2.*HistUpdate)*inner(p,q)-\
            2.*HistUpdate*q)*dx
du = Function(W)
p = Function(V)
...
nn = np.hstack((np.linspace(1e-6,0.02,50), np.linspace(0.02,1e-6,50), np.linspace(1e-6,0.1,100)))

with open('TwoElem.txt', 'a') as f:
    for (i_t,t) in enumerate(nn) :
        u_Lx.t=t*ut
        iter = 1
        toll = 1e-2
        err = 1
        ...
        while err > toll:
            #if iter==1:
            #    HistUpdate.assign(HistOld)
            #else:
            #    assign(HistUpdate,project(en_dens(u_old),WWW))
            Hn = project(en_dens(du), WWW)
            zz = np.maximum(Hn.vector().array(), HistOld.vector().array())
            S0.vector().set_local(zz)
            assign(HistUpdate,S0)
            

            #plot(HistUpdate, title = 'HistU')
            ...
            err_phi = errornorm(p,phi_old, norm_type = 'l2', mesh = None)
